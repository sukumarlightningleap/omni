      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="space-y-1 text-center md:text-left">
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 italic">Content Management</h1>
          ...
          <div className="flex gap-3">
            <button 
              ...
              className="px-6 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700 transition-all flex items-center gap-2 shadow-sm shadow-indigo-200"
            >
              {isSavingConfig ? <Loader2 className="animate-spin" size={16} /> : <Check size={16} />} Save Global Assets
            </button>
          </div>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {SECTIONS.map((section) => (
            <button
              ...
              className={`relative overflow-hidden group p-6 rounded-2xl border-2 transition-all text-left ${
            ...
            >
              <div className="flex justify-between items-start">
                <div className={`p-3 rounded-xl ${activeSection === section.id ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
                  <section.icon size={24} />
                </div>
                {activeSection === section.id && <div className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">Active Section</div>}
              </div>
              <div className="mt-6 space-y-1">
                <h3 className={`text-xl font-bold transition-colors ${activeSection === section.id ? 'text-slate-900' : 'text-slate-500'}`}>
                  {section.label}
                </h3>
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-8">
              <form onSubmit={handleAddImage} className="flex flex-col sm:flex-row gap-4">
                ...
                <button
                  ...
                  className="bg-indigo-600 text-white px-8 py-4 rounded-xl text-xs font-black uppercase tracking-[0.3em] hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 whitespace-nowrap"
                >
                  {isAddingImage ? <Loader2 className="animate-spin" size={16} /> : "Ingest Asset"}
                </button>
              </form>
